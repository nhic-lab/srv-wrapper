...
<img src="docs/assets/dashboard-register.png" alt="Dashboard registration form for a new server, showing fields for server id, host, port, username, auth method, and key path or password." width="640">
...
<img src="docs/assets/dashboard-list.png" alt="Dashboard Servers view in dark mode, showing a registered server selected in the list with its connection details, host key fingerprint, and recent runs in the detail pane." width="640">

### Live

Every `exec` and session in flight, streamed over WebSocket, labelled by the `--agent` value that started it.

<img src="docs/assets/dashboard-live.png" alt="Dashboard Live view showing a command in progress, with the agent label, target, elapsed time, and streaming output." width="640">

### History

Every past run: server id, agent label, command, exit code, timing, and full captured output. Paginated, newest first.

<img src="docs/assets/dashboard-history.png" alt="Dashboard History view listing past runs with exit codes and timestamps, with one run selected showing its full command and output." width="640">
